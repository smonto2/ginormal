#' title
#' @useDynLib GinR, .registration = TRUE
#' @param v intermediate parameters
#' @param method intermediate parameters
...
pbdv_r <- function(v, x, method = "Fortran"){
  if (method == "Fortran") {
    ...
    result <- .Fortran("pbdv", v = v, x = x, dv = dv, dp = dp, pdf = pdf, pdd = pdd, PACKAGE="GinR")
